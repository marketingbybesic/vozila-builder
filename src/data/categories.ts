...
const DIJELOVI_SUBS: Subcategory[] = [
  ...
  },
  {
    ...
    icon: "circledot",
    children: [
      { slug: "ljetne-gume", name: "Ljetne gume", icon: "sun" },
      ...
      { slug: "kompleti-gume-felge", name: "Kompleti (gume + felge)", icon: "package" },
      { slug: "ratkape", name: "Ratkape / poklopci kotača", icon: "shield" },
      { slug: "tpms-senzori", name: "TPMS senzori, ventili i oprema", icon: "gauge" },
      { slug: "distancijeri-prstenovi", name: "Distancijeri i prstenovi", icon: "layers" },
...
export function freeTextModelField(categorySlug: string, subcategory?: string): boolean {
  ...
  // ⚠️ Karlo 01.09.2026 (st.44): Dijelovi/Za viličare — Marka bira iz
...
export function makesForSub(categorySlug: string, subcategory?: string): CarMake[] | null {
  ...
  // Karlo 01.09.2026 (st.44): "Za viličare" — SAMO izvor Viličari (Mehanizacija),
  // isti obrazac kao st.43.
  if (categorySlug === "dijelovi" && subcategory === "za-vilicare") return MEHANIZACIJA_VILICARI_MAKES;
