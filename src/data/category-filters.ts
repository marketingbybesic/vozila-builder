...
const DIJELOVI_FIELDS: FilterField[] = [
  ...
  // ⚠️ Karlo 04.09.2026 (st.60): puni popis Karlovih dimenzija (63 stavke,
  // TOČNIM redoslijedom kako je diktirao — ne abecedno/brojčano sortirano).
  // Mix formata: cijeli mm (100-385), decimalni inčni (5.50-15, npr. teretne/
  ...
  { key: "tireWidth", label: "Širina gume", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: TIRE_FULL_FORM_VRSTE.filter((v) => !TERETNE_C_STYLE_VRSTE.includes(v) && v !== "moto-atv-gume" && v !== "quad-atv-utv-gume" && v !== "aluminijske-felge" && v !== "celicne-felge" && v !== "ratkape"),
    options: [
      { value: "100", label: "100" },
      { value: "105", label: "105" },
      { value: "110", label: "110" },
      ...
      { value: "14", label: "14" },
      { value: "15", label: "15" },
      { value: "30x9.50", label: "30x9.50" },
      { value: "31x9.50", label: "31x9.50" },
      { value: "31x10.50", label: "31x10.50" },
      { value: "31x11.50", label: "31x11.50" },
      { value: "32x11.50", label: "32x11.50" },
      { value: "33x10.50", label: "33x10.50" },
      { value: "33x12.50", label: "33x12.50" },
      { value: "35x12.50", label: "35x12.50" },
      { value: "35x13.50", label: "35x13.50" },
      { value: "35x12.50", label: "35x12.50" },
      { value: "37x12.50", label: "37x12.50" },
      { value: "37x13.50", label: "37x13.50" },
      { value: "37x14.50", label: "37x14.50" },
      { value: "37x12.50", label: "37x12.50" },
  ...
  // ⚠️ Karlo 07.09.2026 (st.82): Quad / ATV i UTV gume — vlastiti popis Širine
  // gume (43 stavke, TOČNIM redoslijedom kako je diktirao). "20x7.00" stoji
  // DVAPUT zaredom u dostavljenom popisu — nije tipfeler, zadržano kako je
  // dano (isti obrazac kao st.60 potvrđeni duplikati). Isti key `tireWidth`
  // kao gornja 2 unosa — RAZLIČIT `vrstaScope` (samo "quad-atv-utv-gume")
  // osigurava međusobnu isključivost.
  { key: "tireWidth", label: "Širina gume", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["quad-atv-utv-gume"],
    options: [
      "165","16x7.00","16x8.00","175","185","18x9.50","18x10","19x7.00","20x7.00","20x7.00","20x9.00",
      "20x10.00","20x11.00","21x7.00","225","22x10.00","22x11.00","22x12.00","22x9.00","23x8.00","235",
      "24x8.00","24x9.00","25x10.00","25x12.50","25x8.00","26x8.00","26x9.00","26x10.00","26x11.00","270",
      "27x9.00","27x10.00","27x11.00","27x12.00","28x9.00","28x10.00","28x11.00","28x12.00","29x9.00",
      "29x11.00","30x10.00","32x10.00",
    ].map((v) => ({ value: v, label: v })) },
  ...
  { key: "tireProfile", label: "Profil gume", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: TIRE_FULL_FORM_VRSTE.filter((v) => !TERETNE_C_STYLE_VRSTE.includes(v) && v !== "moto-atv-gume" && v !== "quad-atv-utv-gume" && v !== "aluminijske-felge" && v !== "celicne-felge" && v !== "ratkape"),
  ...
  { key: "tireProfile", label: "Profil gume", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["moto-atv-gume"],
    options: ["35","40","50","55","60","65","67","70","75","80","85","88","90","100","600","620","630","640","645"]
      .map((n) => ({ value: n, label: n })) },
  // ⚠️ Karlo 07.09.2026 (st.84): Quad / ATV i UTV gume — vlastiti popis Profila
  // gume (5 stavki, TOČNIM redoslijedom kako je diktirao). Isti key
  // `tireProfile` kao gornja 3 unosa — RAZLIČIT `vrstaScope` (samo
  // "quad-atv-utv-gume") osigurava međusobnu isključivost.
  { key: "tireProfile", label: "Profil gume", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["quad-atv-utv-gume"],
    options: ["30","40","60","70","88"]
      .map((n) => ({ value: n, label: n })) },
  ...
  // ⚠️ Karlo 08.09.2026 (st.95b): Ratkape — jedino polje koje OSTAJE u rubrici
  // Dimenzije (izričito zatraženo: "izbriši sve, samo ostavi Promjer (col)").
  // Ratkape NAMJERNO ostaje u ovom (općem) `tireDiameter` unosu — sve OSTALO iz
  // Dimenzije (Širina gume/Profil gume/Indeks nosivosti/Indeks brzine/Runflat)
  // sad isključuje "ratkape" iz svog vrstaScope (vidi ta polja niže).
  { key: "tireDiameter", label: "Promjer (col)", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: TIRE_FULL_FORM_VRSTE.filter((v) => !TERETNE_C_STYLE_VRSTE.includes(v) && v !== "moto-atv-gume" && v !== "quad-atv-utv-gume" && v !== "aluminijske-felge" && v !== "celicne-felge"),
  ...
  { key: "tireRunflat", label: "Runflat", type: "toggle", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: TIRE_FULL_FORM_VRSTE.filter((v) => v !== "aluminijske-felge" && v !== "celicne-felge" && v !== "ratkape") },
  ...
  // ⚠️ Karlo 07.09.2026 (st.93): Čelične felge — SVE IDENTIČNO Aluminijskim
  // felgama (izričito zatraženo: "napravi sve identično"). "celicne-felge"
  // dodana u vrstaScope svih 5 dediciranih unosa niže, I u isključenja svih
  // odgovarajućih "gume"/Indeks/Runflat unosa gore (da se izbjegne dvostruko
  // renderiranje). "Za Marku" label i MAKES popis marki (napredno-form.tsx)
  // isto prošireni na celicne-felge.
  ...
  { key: "tireDiameter", label: "Promjer (col)", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["aluminijske-felge", "celicne-felge"],
  ...
  { key: "tireWidth", label: "Širina felge", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["aluminijske-felge", "celicne-felge"],
    options: [
  ...
  { key: "tireProfile", label: "Broj rupa", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["aluminijske-felge", "celicne-felge"],
  ...
  { key: "tireLoadIndex", label: "Razmak rupa", type: "select", storage: "attr", group: "Dimenzije", scope: ["gume"], vrstaScope: ["aluminijske-felge", "celicne-felge"],
    options: [
  ...
  // Vrstu "Ulja, maziva i adetivi". Odvojen ključ (`oilViscosityList`) od
  // dijeljenog `brandPart` (koji ostaje tekst-polje "Proizvođač dijela" za
  // sve ostale Vrste Dijelova) — isti obrazac kao st.86 label/type split.
  { key: "oilViscosityList", label: "Viskoznost ulja", type: "select", storage: "attr", group: "Detalji", scope: ["ulja-tekucine"], vrstaScope: ["ulja-maziva-aditivi"],
    options: [
      "0W-16","0W-20","0W-30","0W-40","0W-50",
      "5W-20","5W-30","5W-40","5W-50",
      "10W-30","10W-40","10W-50","10W-60",
      "15W-40","15W-50",
      "20W-40","20W-50",
      "25W-40",
      "75W-80","75W-90","75W-140",
      "80W-90",
      "85W-140",
      "32W","46W","68W",
